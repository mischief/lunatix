...
-- The server half wants a scheduler and a namespace; the client half
-- wants neither, so they load where they are used rather than here.
local ok_thread, thread = pcall(require, "los.thread")
local ok_ns, ns = pcall(require, "ns")
if not ok_thread then thread = nil end
if not ok_ns then ns = nil end
